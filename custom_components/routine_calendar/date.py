"""Date platform: one 'last completed' entity per routine.

Setting the entity (via UI or `date.set_value` service) re-anchors the
next due date to `new_value + interval`.
"""
...
_DESCRIPTION = DateEntityDescription(
    key="last_completed",
...
async def async_setup_entry(
    ...
    def _sync_entities() -> None:
        ...
        if new_ids:
            async_add_entities(
                [RoutineLastCompletedEntity(coordinator, rid) for rid in new_ids]
            )
...
class RoutineLastCompletedEntity(DateEntity):
    """Date entity that observes / drives a routine's completion date."""
    ...
    def __init__(
        ...
        self._attr_unique_id = f"{DOMAIN}_{routine_id}_last_completed"
    ...
    def native_value(self) -> date | None:
        ...
        if routine is None:
            return None
        return routine[CONF_LAST_COMPLETED]
    ...
    async def async_set_value(self, value: date) -> None:
        await self._coordinator.async_set_last_completed(self._routine_id, value)
